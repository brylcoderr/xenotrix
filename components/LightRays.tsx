...
const getAnchorAndDir = (origin: string, w: number, h: number) => {
    ...
    switch (origin) {
        case 'top-left':
            return { anchor: [-0.05 * w, -0.05 * h], dir: [1, 0.8] };
        case 'top-right':
            return { anchor: [(1 + outside) * w, -outside * h], dir: [-1, 1] };
        case 'left':
            return { anchor: [-outside * w, 0.5 * h], dir: [1, 0] };
        case 'right':
            return { anchor: [(1 + outside) * w, 0.5 * h], dir: [-1, 0] };
        case 'bottom-left':
            return { anchor: [-outside * w, (1 + outside) * h], dir: [1, -1] };
        case 'bottom-center':
            return { anchor: [0.5 * w, (1 + outside) * h], dir: [0, -1] };
        case 'bottom-right':
            return { anchor: [(1 + outside) * w, (1 + outside) * h], dir: [-1, -1] };
        default: // "top-center"
            return { anchor: [0.5 * w, -0.05 * h], dir: [0, 1] };
    }
};
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Dynamic noise for volumetric effect
  float n1 = noise(coord * 0.005 + iTime * 0.05);
  float n2 = noise(coord * 0.01 - iTime * 0.08);
  float volumetricNoise = mix(n1, n2, 0.5);
  ...
  fragColor = rays1 * 0.4 + rays2 * 0.5;

  if (noiseAmount > 0.0) {
    fragColor.rgb *= (1.0 - noiseAmount + noiseAmount * volumetricNoise);
    // Add some "shimmer"
    fragColor.rgb += noiseAmount * 0.05 * sin(volumetricNoise * 10.0 + iTime);
  }
